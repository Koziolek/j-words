...
const WordView = ({pl = '', kana = '', romanji = '', kanji = '', internalTag = 'div'}) => {
    ...
    return (
        <InternalTag>
            ...
            <samp className='word-result-kana'>{kana}<OptionalWordView text={kanji} prefix=' – '/></samp>
            <samp className='word-result-romanji'><OptionalWordView text={romanji}/></samp>
        </InternalTag>
    );
}
...
const WordForms = ({words = [], wrapperTag = 'div', internalTag = 'div'}) => {
    ...
    return (
        <WrapperTag>
            {
                words.map(word =>
                    <WordView kana={word.hiragana ? word.hiragana : word.katakana}
                              kanji={word.kanji}
        ...
        <div className={mode + '-word-result word-result'}>
            {
                mode === 'bad' ?
                    <>
                        <div>
                            <WordView kana={word.answer}
                                      romanji={toRomaji(word.answer)}
                                      pl={word.pl}
                            />
                        </div>
...
const WordResultComponent = ({
    ...
    return (
        ...
        <div className='word-result'>
            <div>
                <h2>Poprawne</h2>
                {goodWords.map(word => <WordResult word={word} mode='good' key={v4()}/>)}
            </div>
            <div>
                <h2>Błędy</h2>
                <div className='word-result'>
                ...
                {badWords.map(word => <WordResult word={word} mode='bad' key={v4()}/>)}
            </div>
        </div>
...
WordResultComponent.propTypes = {
    goodWords: PropTypes.arrayOf(PropTypes.shape({
        answer: PropTypes.string,
        pl: PropTypes.string,
        words: PropTypes.arrayOf(PropTypes.shape({
            hiragana: PropTypes.string,
    ...
    badWords: PropTypes.arrayOf(PropTypes.shape({
        answer: PropTypes.string,
        pl: PropTypes.string,
        words: PropTypes.arrayOf(PropTypes.shape({
            hiragana: PropTypes.string,
