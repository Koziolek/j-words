...
import { randomWords } from '../word/WordsService';
import GoodBadCounter from '../counter/GoodBadCounter';
import WordResultComponent from '../word-result/WordResultComponent';
...
const LearnModeComponent = () => {
  ...
  useEffect(() => {
    if (state.words.length === 0) {
      initialise(() => randomWords({ count: max })).then((data) => setState(data));
    }
